...
public abstract class Classifier {
   ...
   */
  public static Classifier create(Context context, Model model, Device device, int numThreads, String... labels)
      throws IOException {
    if (model == Model.QUANTIZED_MOBILENET) {
      return new ClassifierQuantizedMobileNet(context, device, numThreads, labels);
    } else if (model == Model.FLOAT_MOBILENET) {
      return new ClassifierFloatMobileNet(context, device, numThreads, labels);
    } else if (model == Model.FLOAT_EFFICIENTNET) {
      return new ClassifierFloatEfficientNet(context, device, numThreads, labels);
    } else if (model == Model.QUANTIZED_EFFICIENTNET) {
      return new ClassifierQuantizedEfficientNet(context, device, numThreads, labels);
    } else {
  ...
  protected Classifier(Context context, Device device, int numThreads, String... labels) throws IOException {
    MappedByteBuffer tfliteModel = loadModelFile(context.getAssets(), getModelPath());
    switch (device) {
      case NNAPI:
